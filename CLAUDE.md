...
### コード修正時の注意点
1. `script.js:170-180` - ゴミの座標計算（中央配置）
2. `script.js:187` - 落下速度調整（10倍遅く設定）
3. `style.css:112-133` - ゴミアイテムの基本スタイル
4. `script.js:22-32` - ゴミの種類定義（渋谷区ルール準拠）

### 新しいゴミ追加方法（渋谷区ルール準拠）
1. `garbageTypes`配列に新オブジェクト追加
   - `type`: 'combustible', 'non-combustible', 'resource', 'bulky'のいずれか
2. 対応するCSSクラス作成（`.garbage-item.{name}`）
3. 絵文字と正確な日本語ラベル設定
